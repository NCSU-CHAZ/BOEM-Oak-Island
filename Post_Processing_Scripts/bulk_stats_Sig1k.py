...
def initialize_bulk(
    ...
    Waves = {
        ...
        "Vavg": pd.DataFrame([]),
        "Wavg": pd.DataFrame([]),
        "MeanDir1": pd.DataFrame([]),
        "MeanSpread1": pd.DataFrame([]),
        "MeanDir2": pd.DataFrame([]),
        "MeanSpread2": pd.DataFrame([]),
        "avgFlowDir": pd.DataFrame([]),
        "Spp": pd.DataFrame([]),
        "Svv": pd.DataFrame([]),
        "Suu": pd.DataFrame([]),
        "Spu": pd.DataFrame([]),
        ...
        "Echo1avg": pd.DataFrame([]),
        "Sv1": pd.DataFrame([]),
        "vertavg": pd.DataFrame([]),
        "sedtime": pd.DataFrame([]),
        "TS": pd.DataFrame([]),
        "botscatt": pd.DataFrame([]),
        "topscatt": pd.DataFrame([]),
        "TopSv1": pd.DataFrame([]),
        "BotSv1": pd.DataFrame([]),
        ...
        "FullV": pd.DataFrame([]),
        "FullW": pd.DataFrame([]),
        "Spp_ast": pd.DataFrame([]),
        "fr_ast": pd.DataFrame([]),
        "STD_ast_p": pd.DataFrame([]),
        "StokesDriftMagnitude": pd.DataFrame([]),
        "StokesDriftDirection": pd.DataFrame([]),
        "StokesTransport": pd.DataFrame([]),
        "Echosmall": pd.DataFrame([]),
    }
...
def load_qc_data(group_path, Waves, echosounder=True):
    ...
    if echosounder:
        Data["CellDepth_echo"] = pd.read_hdf(
            os.path.join(group_path, "CellDepth_echo.h5")
        )
...
def sediment_analysis_vert(Data, Waves, sbe, transmit_length=0.330, vertical_beam=True):
    # ph = 8.1
    # freq = 1000  # kHz
    ...
    Waves["vertavg"] = pd.concat([Waves["vertavg"], vertavg], axis=0, ignore_index=True)

    return Waves, Data


def sediment_analysis(Waves, Data, sbe, transmit_length=0.330):
...
def calculate_wave_stats(
    ...
    if "Echo1avg" in Data:
        E = Data["Echo1"].iloc[i * Nsamp : Nsamp * (i + 1), :]
    else:
        E = Data["VbAmplitude"].iloc[i * Nsamp : Nsamp * (i + 1), :]
    # AST=Data['AST_amp'].iloc[i * Nsamp: Nsamp * (i + 1),:] # try this out

    # Grab mean amplitude for the ensemble
    ...
    Nens = dtens * fs  # number of samples in each ensemble
    dt = 1 / fs
    Chunks = (Nsamp - Nens * overlap - 1) / (Nens * (1 - overlap))
    ...
    Suu, fr = welch_method(U_no_nan, dt, Chunks, overlap)
    ...
    Waves["C"] = pd.concat(
        [Waves["C"], pd.DataFrame([np.nanmean(C)])], axis=0, ignore_index=True
    )
    Waves["Hs"] = pd.concat(
    ...
    P_expanded = np.tile(P_no_nan, (1, Nb))
    ...
    # Handle infinite values by replacing them with NaN and then filling NaNs with zero
    ...
    ma1 = np.nansum(a1.iloc[I].values * SePP.iloc[I].values * df.values, axis=0) / m0
    mb1 = np.nansum(b1.iloc[I].values * SePP.iloc[I].values * df.values, axis=0) / m0

    # Compute average directional spreads
    mdir1 = np.remainder(90 + 180 - r2d * np.arctan2(mb1, ma1), 360)
    mdir1[mdir1 == 270.0] = np.nan
    mspread1 = r2d * np.sqrt(
        np.abs(2 * (1 - (ma1 * np.cos(mdir1 / r2d) + mb1 * np.sin(mdir1 / r2d))))
    )
    ...
    ma2 = np.nansum(a2.iloc[I].values * SePP.iloc[I].values * df.values, axis=0) / m0
    mb2 = np.nansum(b2.iloc[I].values * SePP.iloc[I].values * df.values, axis=0) / m0
    ...
    mspread2 = r2d * np.sqrt(
        np.abs(
            0.5 - 0.5 * (ma2 * np.cos(2 * mdir2 / r2d) + mb2 * np.sin(2 * mdir2 / r2d))
        )
    )
    ...
    stokes_int = (
        fr_rad.values.flatten()
        * k_arr
        * (np.cosh(2 * k_arr * dpth) / (np.sinh(k_arr * dpth) ** 2))
    )

    # Integrate stokes drift over the spectrum using surface elevation spectrum
    Us_mag = np.nansum(stokes_int[I] * SePP.iloc[I].values * df.values, axis=0)
    ...
    a1_f_col = a1_f.reshape(-1, 1)
    b1_f_col = b1_f.reshape(-1, 1)

    # Calculate stokes drift direction using the first order directional spectrum and integrating over frequency space
    Us_x = np.nansum(stokes_int[I] * a1_f_col * SePP.iloc[I].values * df.values, axis=0)
    Us_y = np.nansum(stokes_int[I] * b1_f_col * SePP.iloc[I].values * df.values, axis=0)

    # Calculate stokes drift direction in degrees
    Us_dir = 90 - 180 + np.degrees(np.arctan2(Us_y, Us_x))

    # Calculate transport based on echo avg
    Transport = Us_mag * Eavg  # mass transport per unit width

    # Put the directions and spreads for the waves into Waves structure
    Waves["StokesDriftMagnitude"] = pd.concat(
        [Waves["StokesDriftMagnitude"], pd.DataFrame([Us_mag])],
        axis=0,
        ignore_index=True,
    )
    Waves["StokesDriftDirection"] = pd.concat(
        [Waves["StokesDriftDirection"], pd.DataFrame([Us_dir])],
        axis=0,
        ignore_index=True,
    )
    Waves["StokesTransport"] = pd.concat(
        [Waves["StokesTransport"], pd.DataFrame([Transport])], axis=0, ignore_index=True
    )
    Waves["Echosmall"] = pd.concat(
        [Waves["Echosmall"], pd.DataFrame([Eavg])], axis=0, ignore_index=True
    )
    Waves["MeanDir1"] = pd.concat(
        [Waves["MeanDir1"], pd.DataFrame([np.nanmean(mdir1)])],
        axis=0,
        ignore_index=True,
    )
    Waves["MeanSpread1"] = pd.concat(
        [Waves["MeanSpread1"], pd.DataFrame([np.nanmean(mspread1)])],
        axis=0,
        ignore_index=True,
    )
    Waves["MeanDir2"] = pd.concat(
        [Waves["MeanDir2"], pd.DataFrame([np.nanmean(mdir2)])],
        axis=0,
        ignore_index=True,
    )
    Waves["MeanSpread2"] = pd.concat(
        [Waves["MeanSpread2"], pd.DataFrame([np.nanmean(mspread2)])],
        axis=0,
        ignore_index=True,
    )
    Waves["Spp"] = pd.concat(
        [Waves["Spp"], pd.DataFrame([np.nanmean(Spp.loc[0 : I[-1], :], axis=1)])],
        axis=0,
        ignore_index=True,
    )
    ...
    #         )
    Waves["Svv"] = pd.concat(
        [Waves["Svv"], pd.DataFrame([np.nanmean(Svv.loc[0 : I[-1], :], axis=1)])],
        axis=0,
        ignore_index=True,
    )
    Waves["Suu"] = pd.concat(
        [Waves["Suu"], pd.DataFrame([np.nanmean(Suu.loc[0 : I[-1], :], axis=1)])],
        axis=0,
        ignore_index=True,
    )
    Waves["Spu"] = pd.concat(
        [Waves["Spu"], pd.DataFrame([np.nanmean(Spu.loc[0 : I[-1], :], axis=1)])],
        axis=0,
        ignore_index=True,
    )
    Waves["Spv"] = pd.concat(
        [Waves["Spv"], pd.DataFrame([np.nanmean(Spv.loc[0 : I[-1], :], axis=1)])],
        axis=0,
        ignore_index=True,
    )
...
def save_waves(Waves, save_dir):
    ...
    Waves["fr_ast"].to_hdf(
        os.path.join(save_dir, "Frequencies_AST"), key="df", mode="w"
    )
    ...
    Waves["Hs"].to_hdf(
        os.path.join(save_dir, "SignificantWaveHeight"), key="df", mode="w"
    )
    Waves["Uavg"].to_hdf(
        os.path.join(save_dir, "DepthAveragedEastVelocity"), key="df", mode="w"
    )
    Waves["Vavg"].to_hdf(
        os.path.join(save_dir, "DepthAveragedNorthVelocity"), key="df", mode="w"
    )
    Waves["Wavg"].to_hdf(
        os.path.join(save_dir, "DepthAveragedUpVelocity"), key="df", mode="w"
    )
    Waves["Current"].to_hdf(
        os.path.join(save_dir, "DepthAveragedCurrentVelocity"), key="df", mode="w"
    )
    Waves["MeanDir1"].to_hdf(
        os.path.join(save_dir, "MeanDirection1"), key="df", mode="w"
    )
    Waves["MeanSpread1"].to_hdf(
        os.path.join(save_dir, "MeanSpread1"), key="df", mode="w"
    )
    Waves["MeanDir2"].to_hdf(
        os.path.join(save_dir, "MeanDirection2"), key="df", mode="w"
    )
    Waves["MeanSpread2"].to_hdf(
        os.path.join(save_dir, "MeanSpread2"), key="df", mode="w"
    )
    Waves["avgFlowDir"].to_hdf(
        os.path.join(save_dir, "DepthAveragedFlowDirection"), key="df", mode="w"
    )
    # Waves["Spp_ast"].to_hdf(os.path.join(save_dir, "ASTPressureSpectra"), key="df", mode="w")
    Waves["Echosmall"].to_hdf(os.path.join(save_dir, "SSC"), key="df", mode="w")
    Waves["Spp"].to_hdf(os.path.join(save_dir, "PressureSpectra"), key="df", mode="w")
    Waves["Spu"].to_hdf(
        os.path.join(save_dir, "PressureEastVelCospectra"), key="df", mode="w"
    )
    Waves["Spv"].to_hdf(
        os.path.join(save_dir, "PressureNorthVelCospectra"), key="df", mode="w"
    )
    ...
    Waves["Sv1"].to_hdf(
        os.path.join(save_dir, "VolumetricBackscatter1"), key="df", mode="w"
    )
    ...
    Waves["botscatt"].to_hdf(
        os.path.join(save_dir, "BottomhalfScatterersavg"), key="df", mode="w"
    )
    Waves["topscatt"].to_hdf(
        os.path.join(save_dir, "TophalfScatterersavg"), key="df", mode="w"
    )
    Waves["TopSv1"].to_hdf(
        os.path.join(save_dir, "TopVolumetricBackscatter1"), key="df", mode="w"
    )
    Waves["BotSv1"].to_hdf(
        os.path.join(save_dir, "BotVolumetricBackscatter1"), key="df", mode="w"
    )
    Waves["Pressure"].to_hdf(os.path.join(save_dir, "Pressure"), key="df", mode="w")
    Waves["FullU"].to_hdf(
        os.path.join(save_dir, "FullEastVelocity"), key="df", mode="w"
    )
    Waves["FullV"].to_hdf(
        os.path.join(save_dir, "FullNorthVelocity"), key="df", mode="w"
    )
    Waves["FullW"].to_hdf(os.path.join(save_dir, "FullUpVelocity"), key="df", mode="w")
    Waves["StokesDriftMagnitude"].to_hdf(
        os.path.join(save_dir, "StokesDriftMagnitude"), key="df", mode="w"
    )
    Waves["StokesDriftDirection"].to_hdf(
        os.path.join(save_dir, "StokesDriftDirection"), key="df", mode="w"
    )
    Waves["StokesTransport"].to_hdf(
        os.path.join(save_dir, "StokesTransport"), key="df", mode="w"
    )
